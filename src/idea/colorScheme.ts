...
import { removeHashFromHex } from 'src/util/color'
...
function generateIdeaColorScheme (
  ...
  return {
    ...
    colors: {
      ...
      DIFF_SEPARATORS_BACKGROUND: findEditorColorForFields('panel.background'),
      ...
      FOLDED_TEXT_BORDER_COLOR: findEditorColorForFields('sideBar.foreground'),
    ...
    attributes: {
      ...
      BAD_CHARACTER: {
        value: [
          {
            EFFECT_COLOR: findTokenColorForFields('markup.error') as string
          },
          { EFFECT_TYPE: 2 }
        ]
      },
...
export function convertToIdea (vscodeTheme: CodeTheme): void {
  console.log(
    js2xmlparser.parse(
      'scheme',
      mapToXmlObject(generateIdeaColorScheme(vscodeTheme))
    )
  )
}
